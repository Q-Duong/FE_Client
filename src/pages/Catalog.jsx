...
const Catalog = () => {
  ...
  useEffect(() => {
    const SELECTED_BRAND = selectBrand
      ? selectBrand.replaceAll("-", " ")
      : false;
      ...
      try {
        ...
        const queryBrand =
          filter.brands.length > 0
            ? `brandIds[]=${filter.brands.join("&brandIds[]=")}&`
            : foundBrand
            ? `brandIds[]=${foundBrand.id}&`
            : "";
        const queryCategory =
          filter.categories.length > 0
            ? `categoryIds[]=${filter.categories.join("&categoryIds[]=")}&`
            : "";
        const queryPage = `page=${activePage}&`;

        const queryParams =
          "order=ASC&" + queryBrand + queryCategory + queryPage + search;
                ...
                {categories.map((item) => (
                  ...
                  >
                    <CheckBox
                      label={item.name}
                      onChange={(input) =>
                        filterCategoySelect(input.checked, item)
                      }
                ...
                {brands.map((item) => (
                  ...
                  >
                    <CheckBox
                      label={item.name}
                      onChange={(input) =>
                        filterBrandSelect(input.checked, item)
                      }
